...
class AllConv(object):
    ...
    def build_model(self, X):
        ...
        model.add(Activation('relu'))
        model.add(
            Conv2D(96, (3, 3), strides=(2, 2), padding='same', name='conv3'))
        model.add(Activation('relu'))
        ...
        model.add(Activation('relu'))
        model.add(
            Conv2D(192, (3, 3), strides=(2, 2), name='conv6', padding='same'))
        model.add(Activation('relu'))
        model.add(Dropout(0.5))
        ...
        try:
            ...
            opt = optimizer(
                lr=self.learning_rate, schedule_decay=self.lr_decay)
    ...
    def transform(self, X):
        ...
        output = [
            layer.output for layer in model.layers if layer.name == 'conv9'
        ][0]
        func = K.function(inp + [K.learning_phase()], [output])
        for i in range(int(X.shape[0] / self.batch_size) + 1):
            minibatch = X[i * self.batch_size:min(X.shape[0], (
                i + 1) * self.batch_size)]
        ...
        output = np.reshape(output,
                            (output.shape[0], np.product(output.shape[1:])))
        return output
