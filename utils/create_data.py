...
parser.add_argument(
    "--save_dir", type=str, default="/tmp/data", help="Where to save outputs.")
parser.add_argument(
    "--datasets",
    type=str,
    default="",
    help="Which datasets to download, comma separated.")
...
def get_wikipedia_talk_data():
  ...
  """
    ...
    comments = pd.read_table(
        _download_file(ANNOTATED_COMMENTS_URL), index_col=0, sep='\t')
    ...
    # Process labels
    annotations = pd.read_table(_download_file(ANNOTATIONS_URL), sep='\t')
    ...
    for member in tar_dir.getnames():
        if '_batch' in member:
            ...
            if X is None:
                ...
                X = np.concatenate((X, np.array(batch['data'],
                                                dtype=np.uint8)))
...
def main(args):
    ...
    # First entry of tuple is mldata.org name, second is the name that we'll
    # use to reference the data.
    datasets = [('mnist (original)', 'mnist'),
                ...
                ('vehicle', 'vehicle'),
                ('wine', 'wine'),
                ('waveform ida', 'waveform'),
                ('german ida', 'german'),
                ('splice ida', 'splice'),
                ('ringnorm ida', 'ringnorm'),
                ('twonorm ida', 'twonorm'),
                ('diabetes_scale', 'diabetes'),
                ('mushrooms', 'mushrooms'),
                ('letter', 'letter'),
                ('dna', 'dna'),
                ('banana-ida', 'banana'),
    ...
    if args.datasets:
        subset = args.datasets.split(',')
        datasets = [d for d in datasets if d[1] in subset]

    for d in datasets:
        print(d[1])
        download_mldata(d, args.save_dir)
