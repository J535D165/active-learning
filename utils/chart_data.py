...
flags.DEFINE_string('source_dir', '/tmp/toy_experiments',
...
def plot_results(all_results, score_method, norm, stand, sampler_filter):
    ...
    for k in sorted(all_results.keys()):
        ...
        if (isinstance(k, tuple) and k[fields['score_method']] == score_method
                and k[fields['standardize']] == stand
                and k[fields['normalize']] == norm
                and (sampler_filter is None or sampler in sampler_filter)):
            results = all_results[k]
            n_trials = results['accuracy'].shape[0]
            x = results['data_sizes'][0]
            mean_acc = np.mean(results['accuracy'], axis=0)
            CI_acc = np.std(
                results['accuracy'], axis=0) / np.sqrt(n_trials) * 2.96
            if sampler == 'uniform':
                plt.plot(
                    x,
                    mean_acc,
...
def get_standardize(filename):
    return get_between(filename, '_stand_',
                       filename[filename.rfind('_'):]) == 'True'
