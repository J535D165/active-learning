...
from models.kernel_block_solver import BlockKernelSolver
...
def get_mldata(data_dir, name):
  ...
  """
    ...
    if dataname == "checkerboard":
        X, y = create_checker_unbalanced(
            split=[1. / 5, 4. / 5], n=10000, grid_size=4)
    else:
...
def get_model(method, seed=13):
  ...
  """
    ...
    #   a decision function. [Good point]
    ...
    # These models do not have a grid search. So return them directly.
...
def get_train_val_test_splits(X,
                              y,
                              max_points,
                              seed,
                              confusion,
  ...
  """
    ...
    assert all(y_noise[indices[0:max_points]] == np.concatenate(
        (y_train, y_val, y_test), axis=0))
    return (indices[0:max_points], X_train, y_train, X_val, y_val, X_test,
            y_test, y_noise)
